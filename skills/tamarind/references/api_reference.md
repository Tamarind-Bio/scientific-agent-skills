...
| POST | `/delete-job` | Delete a job and its data. Body: `jobName`. |
...
The advanced gating keys below (`exclude`, `restrictOrgs`, `conditionals`) appear
**only in MCP `getJobSchema`**, not in REST `/tools`. So don't try to hand-derive
what to strip from REST schema keys — they aren't there. The reliable guard on
both surfaces is **`validateJob`** (MCP): it runs `/submit-job`'s exact validation
without submitting and returns the first error.

- **Don't resubmit `validateJob`'s `normalized` output.** It echoes
  platform-internal fields (`submit_method`, `msa`) and UI-only fields; feeding it
  back to `/submit-job` returns `400 Unrecognized setting`. Submit your own clean
  settings (the same ones you validated), not the normalized echo.
- **Platform-internal routing fields** — `submit_method`, `monomer_msa`, `msa` are
  set by the platform. Never pass them.
- **`restrictOrgs: [...]`** (MCP schema only) — the field is gated to specific
  organizations/domains. Don't set it unless your account is in scope.
...
- **File-typed fields with a plain string value are treated as INLINE CONTENT**,
  not a path. To reference an uploaded object or a prior job output, use the path
  forms `{email}/{filename}` or `JobName/path/to/file.ext`.
